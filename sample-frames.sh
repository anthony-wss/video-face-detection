...
for video_file in $video_folder/*.mp4; do
  ...
  # Save to frames/[video_id]/
  mkdir -p frames/$extracted_id
  ffmpeg -i "$video_file" -vf "select='not(mod(n\,$interval))'" -vsync vfr frames/$extracted_id/frame_%03d.png > /dev/null 2>&1
